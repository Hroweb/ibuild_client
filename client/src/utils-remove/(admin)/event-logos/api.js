...
async function deleteEventLogo(id){
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/logos/${id}`, { method: 'DELETE' });
        if (!response.ok) {
...
async function addEventLogo(data){
    try {
        // Perform the API call to delete the logo
        const response = await fetch(process.env.NEXT_PUBLIC_BACKEND_URL + '/api/logos/', {
            method: 'POST',
...
export {getLogos, updateEventLogos, deleteEventLogo, addEventLogo}